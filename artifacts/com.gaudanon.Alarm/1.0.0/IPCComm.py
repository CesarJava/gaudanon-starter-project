...
class IPCComm:
    ...
    def subscribeToIotTopic(self, topicToSubscribe=None, streamEventCallback=None, streamErrorCallback=None, streamClosedCallback=None):
        topic = self.__topicInterpreter(topicToSubscribe)
        ...
        try:
            # Subscription operations return a tuple with the response and the operation.
            response, operation = self.ipcClient.subscribe_to_iot_core(topic_name=topic, qos='1', on_stream_event=onStreamEventCallback,
                                                        on_stream_error=onStreamErrorCallback, on_stream_closed=onStreamClosedCallback)
            #try:
            #    while True:
            #        time.sleep(10)
